              ...
              {filteredMessages.map((chat) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${chat.role === 'clinician' ? 'bg-green-100' : 'bg-purple-100'
                      }`}>
                      <UserIcon className={`h-5 w-5 ${chat.role === 'clinician' ? 'text-green-600' : 'text-purple-600'
                        }`} />
                    </div>
            ...
            <Card className="h-full flex flex-col">
              <CardHeader className="border-b px-4 py-3">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${selectedChatData?.role === 'clinician' ? 'bg-green-100' : 'bg-purple-100'
                    }`}>
                    <UserIcon className={`h-5 w-5 ${selectedChatData?.role === 'clinician' ? 'text-green-600' : 'text-purple-600'
                      }`} />
                  </div>
                  <div>
              ...
              <CardContent className="flex-1 overflow-auto p-4 flex flex-col space-y-4">
                {selectedChatData?.messages.map((message, index) => (
                  ...
                  >
                    <div className={`max-w-[75%] p-3 rounded-lg ${message.sender === 'Pharmacy'
                        ? 'bg-primary text-primary-foreground'
                        : 'bg-muted'
                      }`}>
                      <p>{message.content}</p>
                      <p className={`text-xs mt-1 ${message.sender === 'Pharmacy'
